...
本仓库**没有真实券商下单连接**，也不允许加入（`AGENTS.md` 第 6 节）。
...
```json
{"strategy_id": "zero_dte_timing_v3", "symbol": "SPY", "direction": "LONG", "contract": "US.SPY260916C600000", "max_qty": 1}
```
...
| 到达强制入场时刻 | 心跳直接生成买入意图，不等 K 线，允许宽点差（原因 `must_trade_deadline`） |
...
python3 -m custody dryrun --symbol US.QQQ --direction LONG --contract US.QQQ260916C705000 \
  --db /tmp/custody-dryrun.sqlite [--strategy zero_dte_timing_v3] [--intent-only] [--once] [--wxpusher-spt "$CUSTODY_WXPUSHER_SPT"]
```
...
- 数据库 schema 为 v3；旧版运行时创建的 SQLite 文件会被拒绝，换一个 `--db` 路径即可。
...
如果将来要接入，需要实现 `custody/ports.py` 的 `Broker`：`submit`（用稳定的 client order id 映射 BUY_OPEN / SELL_CLOSE 限价单，并保证只平已有持仓）、
`cancel`（只确认撤单请求）、`lookup`（重启 / 超时后对账）。首次买入成交回报必须带实际成交时间、当时的正股价格和成交均价。
接入前策略状态必须是 `accepted`。
