...
- **train/custody (canonical)** → the frozen **DTE≤4** paired `custody-train-dte4` slice at `/workspace/pi-jobs/custody-train-dte4/out/custody-train-dte4` (zip + `.sha256` beside it): 19 sessions 2026-08-17 → 2026-09-11, all 10 underlyings, **124** paired cases (114 option contracts, **36 true 0DTE**), max **DTE 4**, zip SHA256 `d72e190e6467f780782f809302a51f9dc8d6f72a00885c2dfb1aa96bf65c51f0`. Underlying 1m is reused from the v2 Release; only the option legs are fetched once from read-only OpenD. **Calendar overlap is not quality:** the parent builder window (2026-05-14 → 2026-09-11) overlaps v2, but OpenD lost the pre-2026-08-21 weeklies, so the full builder output spans DTE 0…99 and is not formal train. Replay with `python3 -m custody eval-session --slice /workspace/pi-jobs/custody-train-dte4/out/custody-train-dte4`.
- **raw/opend cache (NOT train)** → `custody-train-v2window-paired` (830 cases, DTE 0…99, zip SHA256 `b3536cb6a1e9eb00103cbb43442b3101679e879f3fe2b6db23dd4e490810a8c6`), the parent cache for the DTE≤4 filter. Built with `python3 -m custody fetch-train --out out/custody-train-v2window-paired --v2-dir <eval-data-v2>`. Long-DTE rows are a data-availability artifact; **do not cite as train or alpha evidence**.
...
**Grow the formal train while chains exist:** freeze each new session's front-expiry (DTE≤4) paired underlying+option 1m bars the same day and append them to the canonical set; OpenD drops old weeklies, so the set only grows with a daily freeze (see `CANONICAL_TRAIN_DIR` / `PROSPECTIVE_FREEZE_NOTE` in [`custody/train_window.py`](custody/train_window.py)).
...
# canonical formal train (DTE<=4, frozen): replay only, no fetch
...
# raw/parent cache rebuild (read-only OpenD, once) then filter dte<=4
