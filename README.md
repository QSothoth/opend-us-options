...
- **第二条策略线：保证入场线（2026-09-20，用户决定，版本号 `open_hold_*` 独立管理）**。唯一硬约束是**开盘 1 小时内必须买入一单**，用于某些标的上「宁可买错也要建仓」的场景；在这个约束下入场时刻与方式都可调。它不适用「优于对照组」的门槛，改按[盈亏平衡命中率](docs/STANDARD.md#9b-不择时执行策略的判定open_hold-引擎2026-09-20-起)判定，结论最高 PROVISIONAL，不得用于 `--mode live`。
  - [`open_hold_v1`](docs/STRATEGY.md#研究轮次-r212026-09-20运行前登记不择时执行策略-open_hold)（R21）：09:31 固定买入、持有到强制平仓。训练集盈亏平衡命中率 72.8%、样本外估计 76.4%、验证集 63.0%——三个口径一致，**稳健**。方向对时 +47.2%，但 47% 的 case 亏掉九成以上权利金，最差一笔 −100.7%。
  - [`open_hold_v2`](docs/STRATEGY.md#研究轮次-r222026-09-20运行前登记保证入场线窗口内择时-vs-固定时刻)（R22）：沿用 v6.6 的全套入场与离场规则，加 60 分钟兜底（无信号则强制买入）。训练集 57.9%，**但留一天交叉验证的样本外估计不盈亏平衡、验证集读 78.0%——这个数字不稳健，按 78% 而非 58% 规划**。风险显著更低：亏九成以上的 case 降到 **0**，最差一笔 −84.9%。
  - 选哪个取决于上游命中率：命中率 ≳80% 时 v1 更优（不做离场决策，方向对时吃满），60–80% 时 v2 更优（风险低得多）。**这条线本是为高命中率标的准备的，而在高命中率区间反而是更简单的 v1 更合适。**
