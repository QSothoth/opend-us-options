...
def create_app(service, token, clock=None):
    ...
    def view(j):
        return {'job_id':j['id'],'state':j['state'],'mode':j['mode'],**j['request'],'strategy_hash':j['strategy']['sha256'],'strategy_status':j['strategy']['status'],'position_qty':j['position_qty'],'attention':j['attention'],'entry_reason':j['entry_reason'],'exit_reason':j['exit_reason'],'flatten_at':j['flatten_at']}
