...
class Rule:
    ...
    soft_stop_daily_atr:float=0.
    soft_stop_grace_minutes:int=0
    soft_stop_until_best_gain_daily_atr:float=.15
    stall_minutes:int=0
    stall_min_gain_daily_atr:float=.1
    late_trail_daily_atr:float=0.
    late_trail_start:int=900
    flatten_minute:int=945
    def __post_init__(self):
        ...
        for name in ['structure_activate_daily_atr','expanded_trailing_daily_atr','trail_expand_at_daily_atr','fail_required_progress_daily_atr']:
            if not math.isfinite(getattr(self,name)) or getattr(self,name)<0:raise ValueError(name)
        if type(self.fail_after_minutes)!=int or not 0<=self.fail_after_minutes<=120:raise ValueError('fail clock')
        for name in ['soft_stop_daily_atr','soft_stop_until_best_gain_daily_atr','stall_min_gain_daily_atr','late_trail_daily_atr']:
            if not math.isfinite(getattr(self,name)) or getattr(self,name)<0:raise ValueError(name)
        for name in ['soft_stop_grace_minutes','stall_minutes']:
            if type(getattr(self,name))!=int or not 0<=getattr(self,name)<=120:raise ValueError(name)
        if not 780<=self.flatten_minute<=945 or not 780<=self.late_trail_start<=945:raise ValueError('late clock')
...
def daily_atr(cube,day,lookback=20):
    past=[]
    for sym,g in day.groupby('symbol'):
        ...
        x=g[['date','symbol']].copy();x['daily_atr']=tr.rolling(lookback,min_periods=lookback).mean().shift(1).to_numpy();past.append(x)
...
REASONS={0:'no_entry',1:'scheduled_flatten',2:'hard_stop_gap',3:'hard_stop_intrabar',4:'trailing',5:'max_hold',6:'structural',7:'failed_followthrough',8:'indicator_reversal',9:'soft_failure_loss',10:'winner_stall'}
...
def replay(signals,execution,ad,rule=Rule(),job=None,trace=False,source='common_1m_aggregation',feature_override=None,record_mtm=True):
    ...
    mtm=np.zeros((n,390)) if record_mtm else None;hist=[];flatten=np.minimum(meta.flatten.to_numpy(),rule.flatten_minute)
    best_minute=np.full(n,-1)
    if job:
    ...
    for i in range(390):
        ...
        if (end-570)%step==0:
            ...
            holding=(state==State.IN)&native_valid
            new_best=holding&(sign*price>best);best_minute[new_best]=end
            best[holding]=np.maximum(best[holding],sign[holding]*price[holding])
            why=np.zeros(n,np.int8)
            best_gain=best-sign*en;profit=sign*(price-en);elapsed=end-(570+ei)
            if rule.soft_stop_daily_atr:
                why[holding&(elapsed>=rule.soft_stop_grace_minutes)&(best_gain<rule.soft_stop_until_best_gain_daily_atr*d_atr)&(profit<=-rule.soft_stop_daily_atr*d_atr)]=9
            if rule.structure_minutes:
                ...
                why[holding&(why==0)&(best_gain>=rule.structure_activate_daily_atr*d_atr)&(sign*(price-level)<=0)]=6
            ...
            if rule.late_trail_daily_atr and end>=rule.late_trail_start:trail=np.minimum(trail,rule.late_trail_daily_atr)
            why[holding&(why==0)&(best-sign*price>=trail*d_atr)]=4
            if rule.stall_minutes:
                why[holding&(why==0)&(best_gain>=rule.stall_min_gain_daily_atr*d_atr)&(profit>0)&(end-best_minute>=rule.stall_minutes)]=10
