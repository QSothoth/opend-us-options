...
- 唯一评测入口：`python3 -m custody evaluate --strategy <id> --dataset <目录> --out reports/<id>/<dataset>/`。主口径方向对称加权，主指标盈亏比；结论只看门槛，完成分不是门槛；比较版本用综合分（盈亏比、利润因子、方向对 / 不利时的表现加权，不买入按 0 收益计入，见 [STANDARD](docs/STANDARD.md) 第 10 节）；整体比较保留全部 case（未入场贡献零）。
