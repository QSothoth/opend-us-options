...
5. **注册文件不可变**：`custody/strategies/index.json` 固定每个文件的 SHA256。改任何参数 = 新的 `strategy_id`（例如 `zero_dte_timing_v2`）+ 新文件，旧文件不动。
6. **状态**：`candidate`（可 dryrun / paper）→ `accepted`（只有评测结论为 ACCEPT 才能改成这个；live 模式只接受 accepted）→ `retired`。
7. **当前唯一策略**：`zero_dte_timing_v1`，状态 `candidate`，说明见 `docs/STRATEGY.md`。
8. **不做大规模搜参**：新版本只能做少量有明确理由的结构性改动；每个尝试过的变体及其评测结果都要记进 `docs/STRATEGY.md` 的研究记录。
...
- **门槛**：G1 完成率 100%；G2 没有偷看未来数据；G3 平均每笔收益好于对照组；G4 盈亏比 ≥ 2.0；G5 逆势单边时亏得比对照组少；G6 方向正确时平均赚钱。
- **结论等级**：INVALID（G1/G2 不过）/ REJECT（G3–G6 有不过）/ PROVISIONAL（门槛全过但场景覆盖不足或样本外 < 20 个交易日）/ ACCEPT。
...
custody/
  ...
  strategies/             注册的策略参数文件 + index.json（不可变）
