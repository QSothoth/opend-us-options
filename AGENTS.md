...
- **例外，2026-09-20 用户决定**：另设一类**不择时的执行策略**（`open_hold` 引擎），在开盘后的固定分钟买入、收盘前平仓，供上游方向命中率足够高时使用。它不做择时，因此不适用「优于对照组」的门槛（G3 / G5 / G7 / G8 / G9 / G10 / G12 都以对照组为参照，而它本身就是对照组）；它的判定看**自身盈亏平衡命中率**，见 [STANDARD](docs/STANDARD.md)。择时策略的门槛与结论不变。
